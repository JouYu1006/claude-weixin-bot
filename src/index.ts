...
function shouldSearch(text: string): string | null {
  ...
  if (triggers.some(kw => t.includes(kw))) return t;

  // Auto-detect: contains URL → user sharing a link, search for context
  if (/https?:\/\//.test(t)) return null; // don't search URLs, they'll be fetched directly
...
function optimizeSearchQuery(raw: string): string {
  ...
  if (isWeather) {
    ...
    if (cleanChinese.length > 2) q += ` ${cleanChinese}`;
  } else if (isSport) {
    q = engWords ? `${engWords} sports` : `sports news ${dateStr}`;
  } else if (isTech && isNews) {
    q = `latest technology news ${dateStr}`;
    if (engWords) q = `${engWords} ${q}`;
  } else if (isNews) {
    q = `latest news ${dateStr}`;
    if (engWords) q = `${engWords} ${q}`;
  } else if (isTech) {
    q = engWords ? `${engWords} technology` : `technology`;
    if (cleanChinese.length > 2 && cleanChinese !== engWords) q += ` ${cleanChinese}`;
  } else {
    // General: merge English words + cleaned Chinese, deduplicating
    const parts = new Set<string>();
    if (engWords) engWords.split(/\s+/).forEach(w => parts.add(w.toLowerCase()));
    cleanChinese.split(/\s+/).forEach(w => { const l = w.toLowerCase(); if (!parts.has(l)) parts.add(w); });
    q = [...parts].join(" ");
  }
...
async function performSearch(query: string): Promise<SearchResult[]> {
  ...
  try {
    ...
    let b2; while ((b2 = blockRe.exec(html)) !== null && results.length < 8) {
      const body = b2[1];
      ...
      if (titleM) {
        ...
        if (title && title.length > 3 && !/baidu\.com|百度/.test(title)) results.push({ title, snippet, url });
      }
    }
...
function isNewsWorthy(url: string): boolean {
  ...
  try {
    ...
    for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
      ...
      const resp = await client.chat.completions.create({
        ...
        top_p: 0.95,
        tools: AGENT_TOOLS,
        ...(CLAUDE_MODEL.includes("v4-pro") ? { reasoning_effort: LLM_REASONING } as Record<string, unknown> : {}),
      });
